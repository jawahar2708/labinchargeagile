...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadWorkspace(team, db) {
        ...
        badge.className   = "badge " + (approvalStatus === "Approved" ? "status-resolved" : "status-pending");
    ...
    window.loadWorkspaceGlobal = loadWorkspace;
        ...
        document.getElementById("closeAddRequestModal").addEventListener("click", closeAddRequestModal);
        document.getElementById("cancelAddRequest").addEventListener("click", closeAddRequestModal);
        addModal.addEventListener("click", e => { if (e.target === addModal) closeAddRequestModal(); });
        ...
        addForm.addEventListener("submit", function (e) {
            ...
            if (!item) valid = showFieldErr("reqItemName", "reqItemErr", "Item name is required.");
            else if (item.length < 3) valid = showFieldErr("reqItemName", "reqItemErr", "Must be at least 3 characters.");
            if (isNaN(qty) || qty < 1) valid = showFieldErr("reqQty", "reqQtyErr", "Enter a valid quantity (min 1).");
            else if (qty > 999) valid = showFieldErr("reqQty", "reqQtyErr", "Quantity cannot exceed 999.");
            if (!date) valid = showFieldErr("reqDate", "reqDateErr", "Required by date is required.");
            else if (date < today) valid = showFieldErr("reqDate", "reqDateErr", "Date must be today or in the future.");
            if (!purpose) valid = showFieldErr("reqPurpose", "reqPurposeErr", "Purpose / notes is required.");
            else if (purpose.length < 10) valid = showFieldErr("reqPurpose", "reqPurposeErr", "Please provide at least 10 characters.");
